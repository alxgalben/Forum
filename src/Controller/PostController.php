...
class PostController extends AbstractController
{
    ...
    /**
     * @Route("/close-poll/{id}", name="close-poll")
     */
    public function closePoll(Poll $poll, EntityManagerInterface $em): Response
    {
        $totalVotes = 0;
        foreach ($poll->getChoice() as $choice) {
            $totalVotes += $choice->getVote();
        }

        foreach ($poll->getChoice() as $choice) {
            $percentage = $totalVotes > 0 ? ($choice->getVote() / $totalVotes) * 100 : 0;
            $choice->setPercentage($percentage);
            $em->persist($choice);
        }
